...
class ScreenGame:
    def __init__(self, game):
        ...
        self.welcome_background = pygame.image.load('assets/decoration/SUPER_scaled.png')
        self.welcome_background = pygame.transform.scale(self.welcome_background, (WIDTH, HEIGHT))

        # bouton pour le welcome menu
        self.press_spacebar = ImageWithText(self.game.screen,'cadre.png', "PRESS SPACEBAR KEY", (WIDTH // 2,  HEIGHT * 2.5 // 3), True, (400, 80), font_size=36)
    ...
    def draw_welcome_page(self):
        """menu d'acceuil"""
        self.game.screen.blit(self.welcome_background, (0, 0))
        self.press_spacebar.draw()
        ...
        while(waiting):
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if self.press_spacebar.rect.collidepoint(event.pos):
                        waiting = False
    ...
    def load_game(self):
        self.game.audio_manager.play_sound('level_start_effect')
        self.game.map = MapManager(self.game)
        self.game.map.load_map( self.level_selected, LEVELS[self.level_selected]['path'])
        self.game.audio_manager.load_bgm_game(LEVELS[self.level_selected]['song'])
    ...
    def choose_level(self,mouse_pos):
        ...
        for level, button in self.levels.items():
            if button.rect.collidepoint(mouse_pos) and not button.isSelected:
                ...
                self.game.audio_manager.play_sound('map_select_effect')
